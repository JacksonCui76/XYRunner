...
class Test_Main():
    ...
    def test_api(self, caseInfo):
        ...
        #session准备
        sessionUrl = CP.server + Utils.get_dict_value(caseInfo['session'])
        sessionMethod = Utils.get_dict_key(caseInfo['session'])
        SessionData = caseInfo['SessionData']
        isSession = Utils.get_dict_value(caseInfo['session'])
