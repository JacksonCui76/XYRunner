        ...
        if '&' in info:
            ...
            for key_args in info_list:
                ...
                if '[' not in key_args and '//' not in key_args and '@{' not in key_args:
                    key_args_dict = {}
                    key = re.findall(".*?(.*?)\(", key_args)[0]
                    value = re.findall("\((.*)\)", key_args)[0]
    ...
    def check_case_data(cls, Caselist):
        ...
        if len(Caselist[0]) > 12:
            for li in Caselist:
                ...
                # session
                elif type(li['session']) != dict and li['session'] is not None:
                    logger.warning(
                        f"No.{li['CaseID']}'s session Not converted to dictionary, please check the configuration file and select the corresponding column!")
                    count += 1

                # SessionData
                elif type(li['SessionData']) != dict and li['SessionData'] is not None:
                    logger.warning(
                        f"No.{li['CaseID']}'s SessionData Not converted to dictionary, please check the configuration file and select the corresponding column!")
                    count += 1
                ...
                # AssertType
                elif li['AssertType'] not in ('response', 'None', 'length', 'rp', 'len'):
