...
class DoRequests:
    def __init__(self, SessionUrl, SessionMethod, SessionData, session=None):
    ...
    def isSession(self):
        ...
        if self.SessionMethod in ('GET', 'get', 'Get'):
            self.session.get(url=self.SessionUrl, params=self.SessionData)
        elif self.SessionMethod in ('POST', 'post', 'Post'):
            self.session.post(url=self.SessionUrl, data=self.SessionData)
